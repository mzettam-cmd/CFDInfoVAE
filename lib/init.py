...
def init_env(Re=100):
    ...
    """
    from configs.infovae import InfoVAE_config
    assert(Re == InfoVAE_config.Re), print(f"ERROR: Please match the config of vae {InfoVAE_config.Re}!")
    ...
    try:
        if not os.path.exists(datfile):
            try:
                ...
                st = time.time()
                urllib.request.urlretrieve('https://zenodo.org/records/10501216/files/Re100alpha10_newData_150000.hdf5?download=1', 
                                        datfile)
                et = time.time()
                ct = et - st 
