...
from lib.runners import infoVAERunner, latentRunner
...
def vis_temporal_Prediction(
    ...
    Args:
        ...
        vae         : (lib.runners.infoVAERunner) The module for employed VAE     
...
#-----------------------------------------------------------
#-----------------------------------------------------------
def vis_temporal_Prediction_InfoVAE(
                            vae,   # 🔥 on garde "vae" pour compatibilité
                            predictor,
                            model_type,
                            if_loss     = True, 
                            if_evo      = True,
    ...
    """
    Visualisation of the temporal-dynamics prediction results (InfoVAE version)

    Args:

        vae         : (infoVAERunner) InfoVAE model (compatible API with VAE)
        predictor   : latent-space temporal-dynamics model
        model_type  : (str) model type (easy/self/lstm)

        Flags: same as vis_temporal_Prediction
    """
    ...
    figPath   = pathsBib.fig_path + model_type + '/InfoVAE/'
    case_name = predictor.filename
    datPath   = pathsBib.res_path + case_name + '.npz'
    
    print("#"*30)
    print(f"Start InfoVAE visualisation:\nSave Fig to:{figPath}\nLoad data from:{datPath}")
    ...
    # ---------------- LOSS ----------------
    if if_loss:
        plot_loss(predictor.history,
                  save_file=figPath + "loss_" + case_name + '.jpg')
        print(f'INFO: Loss Evolution Saved!')

    # ---------------- LATENT EVOLUTION ----------------
    if if_evo and (g is not None and g.size > 0) and (p is not None and p.size > 0):
        plot_signal(g, p,
                    save_file=figPath + "signal_" + case_name + '.jpg')
        print(f"INFO: Prediction Evolution Saved!")

    # ---------------- ERROR HORIZON ----------------
    if if_window and (e is not None and e.size > 0):
        plot_pred_horizon_error(
            e,
            colorplate.blue,
            save_file=figPath + "horizon_" + case_name + '.jpg'
        )
        print(f"INFO: l2-norm error Horizon Saved!")
    ...
    if if_pmap_s and (pmap_g is not None) and (pmap_p is not None):
        plotSinglePoincare(
            planeNo, i, j,
            pmap_p, pmap_g,
            lim_val, grid_val,
        ...
        print(f"INFO: Single Poincare Map Saved!")

    if if_pmap_all and (pmap_g is not None) and (pmap_p is not None):
        plotCompletePoincare(
    ...
    if if_snapshot and (p is not None and p.size > 0) and (g is not None and g.size > 0):

        InfoVAErec, pred = make_physical_prediction(
            vae=vae,
            pred_latent=p,
            true_latent=g,
            device=vae.device
        ...
        stepPlot = int(predictor.config.in_dim + 1)
        ...
        predFieldFigure(
            ...
            vae.mean,
            stepPlot=stepPlot,
            model_name=model_type + "_InfoVAE",
            save_file=figPath + "recSnapShot_" + case_name + '.jpg'
        )

        print(f"INFO: Reconstructed Snapshot Saved!")
