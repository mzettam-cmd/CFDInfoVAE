...
def vis_infovae(modes_file, training_file):
    """
    Visualisation of the InfoVAE results 

    Args:   
        modes_file      : The file saving the post-processed latent space and modes
    ...
    path = pathsBib.fig_path + 'InfoVAE/'
    Path(path).mkdir(exist_ok=True)

    # Try to plot training history
    try:
        plot_training(training_file)
    except:
        print(f'Warning: {training_file} could not be loaded')

    # Load InfoVAE post-processed file
    with h5py.File(modes_file, 'r') as f:
        print("Keys: %s" % f.keys())
        temporalModes = f['vector'][:, :]          # mu train
        temporalModes_test = f['vector_test'][:, :]# mu test
        modes = f['modes'][:, :, :, :]             # spatial modes
        order = f['order'][:]
        Ecum = f['Ecum'][:]
        NLvalues = f['NLvalues'][:]
        NLmodes = f['NLmodes'][:, :, :, :]

    # Sampling frequency: Re40 = 1, Re100 = 5
    fs = 1 if 'Re40' in modes_file else 5

    # Plot Non-linear modes
    plotNLmodeField(NLmodes, NLvalues, path)
    # Plot complete spatial modes
    plotCompleteModes(modes, temporalModes, modes.shape[0], fs, order, path)
    # Plot latent temporal series
    ...
    print("INFO: InfoVAE visualisation done!")